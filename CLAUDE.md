...
- `src/pages/Dashboard.tsx` — main 3-column view (campaigns → ad sets/ads → detail panel) + recommendations strip. Has a `DateRangeBar` at the top (preset/custom start-end filter) that drives every total, chart and recommendation below it. Expanding an ad set also shows its notes, a mini "which ads to scale/pause/close and why" summary, and an ad-set-level CPL trend chart
- `src/components/` — StatusBadge, Sparkline, AdDetailPanel (Scale/Stop-Close actions, notes), RecommendationsPanel, DataImport, ManualEntryGrid, ClientSettings, NotesField (auto-saving textarea used for ad set / ad notes), DateRangeBar (preset + custom date-range filter)
...
## Change Log
- **2026-09-23 (4)** — Manual close/reopen + date-range filtering:
  - Added `closed_date`/`close_reason` to `Ad`. Closing an ad (from `AdDetailPanel`'s "Stop / Close" button, `closeAd()` in `src/lib/store.ts`) requires a reason, sets status to `killed`, and freezes `daysActive` as of `closed_date` instead of counting to today. `reopenAd()` clears both fields and sets status back to `active`.
  - `AdDetailPanel` regained manual Scale/Stop-Close controls (previous version dropped them after a layout-overlap bug on 2026-09-23 (2)) — this time they sit below the stats grid with no overlap. "Scale" appends a `[Scaled YYYY-MM-DD]` marker to the ad's notes rather than changing status (there's no separate "scaled" state in the data model). A closed ad shows its close date/reason and a "Reopen" link instead of the action buttons.
  - New `DateRangeBar` component (`src/components/DateRangeBar.tsx`) at the top of `Dashboard.tsx`: All time / Last 7 / 14 / 30 days presets plus a custom start-end picker. Filters every ad's entries (`filterEntriesByRange()` in `metrics.ts`) before metrics are computed, so totals, CPL, flags and recommendations all reflect the selected window. `daysActive`/`recommend()` now take an optional `asOf` date (defaults to real today) driven by the range's end date, so viewing a past range doesn't inflate day counts — a manually closed ad's own `closed_date` still wins over `asOf` either way.
  - Supabase schema: `closed_date date` and `close_reason text` added to `ads`. **Action needed**: run the migration block at the bottom of `supabase/schema.sql` (`alter table ads add column if not exists closed_date/close_reason`) in the Supabase SQL editor — the live database predates this change.
